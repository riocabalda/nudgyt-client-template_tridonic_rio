...
function AlertUI({
  ...
  return (
    <div
      className={cn(
        'fixed top-[60px] lg:top-auto lg:bottom-0 left-0 w-full z-1 flex items-center justify-center p-4 pr-[50px] lg:h-[48px] bg-success bg-white text-foreground text-sm lg:text-base break-all',
        variant === 'success' && 'bg-success text-success-foreground',
        variant === 'error' && 'bg-destructive text-destructive-foreground',
        variant === 'info' && 'bg-gray-500 text-success-foreground'
      )}
    >
      {message}
      <button
